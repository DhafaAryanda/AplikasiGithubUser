...
import com.example.myapplication.databinding.FragmentListFollowBinding
...
class ListFollowFragment : Fragment(), ListUserAdapter.OnUserItemClick {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        when (page) {
            1 -> mainViewModel.getFollowers(username ?: "")
            2 -> mainViewModel.getFollowing(username ?: "")
        }
    }
    ...
    private fun initObserver() {
        ...
        mainViewModel.toastText.observe(viewLifecycleOwner) { toastText ->
    ...
    private fun setListFollowers(list: List<UserItem>?) {
        with(binding) {
            if (list.isNullOrEmpty()) {
                tvEmptyState.apply {
                    text = when (page) {
